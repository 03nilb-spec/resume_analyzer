...
- Gemini/Gemma model priority is configured with GEMINI_MODEL_PRIORITY; default order is `gemini-3.1-flash-lite` -> `gemma-4-26b-a4b-it` -> `gemma-4-31b-it` -> `gemini-2.5-flash-lite` -> `gemini-2.5-flash`.

## Free-Tier Rate Limit Protection

- Default Google AI model chain is `gemini-3.1-flash-lite` -> `gemma-4-26b-a4b-it` -> `gemma-4-31b-it` -> `gemini-2.5-flash-lite` -> `gemini-2.5-flash`.
...
- The successful AI model is returned in `aiInsights.model`; mock fallback returns `mock`.
